...
type pathFinder = func(g *graphParams, r *RestrictParams,
	...
	amt lnwire.MilliAtom, finalHtlcExpiry int32) (
...
// the source to the target node of the path finding attempt. It is assumed
// that any feature vectors on all hops have been validated for transitive
// dependencies.
func newRoute(sourceVertex route.Vertex,
	...
	for i := pathLength - 1; i >= 0; i-- {
		...
		// We'll calculate the amounts, timelocks, and fees for each
		// hop in the route. The base case is the final hop which
		// includes their amount and timelocks. These values will
		// accumulate contributions from the preceding hops back to the
		// sender as we compute the route in reverse.
		var (
...
func getOutgoingBalance(node route.Vertex, outgoingChans map[uint64]struct{},
	bandwidthHints map[uint64]lnwire.MilliAtom,
	g routingGraph) (lnwire.MilliAtom, lnwire.MilliAtom, error) {

	var max, total lnwire.MilliAtom
	cb := func(edgeInfo *channeldb.ChannelEdgeInfo, outEdge,
		...
		if !ok {
			bandwidth = lnwire.NewMAtomsFromAtoms(
				edgeInfo.Capacity,
			)
		}
