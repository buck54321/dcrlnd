...
func ChannelCapacity(capacity dcrutil.Amount) OptionalMsgField {
...
type Config struct {
	...
	// notified when a certain peer comes online, allowing it to retry
...
func New(cfg Config, selfKey *secp256k1.PublicKey) *AuthenticatedGossiper {
...
func (d *AuthenticatedGossiper) ProcessLocalAnnouncement(msg lnwire.Message,
	source *secp256k1.PublicKey, optionalFields ...OptionalMsgField) chan error {
...
func calculateSubBatchSize(totalDelay, subBatchDelay time.Duration,
	...
	subBatchSize := (batchSize*int(subBatchDelay) + int(totalDelay) - 1) /
		int(totalDelay)
...
func (d *AuthenticatedGossiper) retransmitStaleAnns(now time.Time) error {
	...
	err := d.cfg.Router.ForAllOutgoingChannels(func(
		...
		if !edge.MessageFlags.HasMaxHtlc() {
			...
			edge.MaxHTLC = lnwire.NewMAtomsFromAtoms(info.Capacity)
